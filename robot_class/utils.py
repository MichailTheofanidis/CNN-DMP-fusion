...
# Returns a normalized time vector
def time_vector(duration, step):
    return np.linspace(0, duration, step)


# Convert to radians
def radiant(q):
    return np.array([math.radians(x) for x in q])
...
# Simple interpolation in joint space
def interp_trajectory(q_init, q_target, time):
    return np.linspace(q_init, q_target, len(time))
